...
Open the GUI only when the user asks to record. Before starting, choose a **Capture target**, check the **Current target** line, and inspect the live **Target preview**. The picker supports either a named application window or **Entire screen** for a specific display. A window target stays bound to that window handle and can be on any monitor. An entire-screen target captures everything visible on the selected display, including other overlapping apps, so confirm the correct display and keep sensitive content off it. The preview is only a live in-app check; it is not stored in the session. Do not start a screenshot-bearing recording with automatic foreground capture, because an unrelated foreground window can contaminate the evidence.

For a controlled CLI recording, pass `--target-process`, `--target-title`, the unambiguous `--target-handle`, or `--target-screen primary` (or a display device name). When controlling the in-app browser, prefer an exact browser-native screenshot and submit it with `image <absolute-path><TAB><description>`; use native window capture for other Windows apps. If an event reports `target-screen-fallback`, ensure the target was visible and unobstructed before relying on that image.
